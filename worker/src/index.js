...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Tạo/phát lại link mời XUYÊN NHÓM — chỉ Ban cán sự lớp (isClassCommittee
      // kiểm bên trong postDanhBaMoi). Nhận roster_id chứ không phải member_id:
      // người chưa đăng nhập có thể còn chưa có dòng members nào.
      if ((m = pathname.match(/^\/api\/danh-ba\/(\d+)\/moi$/)) && method === 'POST') {
        return postDanhBaMoi(request, env, me, Number(m[1]));
      }
