...
--
-- Cột `bang_28` đếm theo DANH SÁCH TÊN chứ không đếm tất cả bảng trong lược đồ.
...
WITH d AS (
  SELECT
    (SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN (
       ...
       'plans','push_subscriptions','rate_events','roster','sessions','thong_bao',
...
SELECT
  ...
  migration  || '/12'                     AS migration_12,
  COALESCE(truong_nhom, '(chưa có)')      AS truong_nhom,
  CASE WHEN bang = 28 AND hoc_vien = 134 AND nhom = 10 AND dien_thoai = 90
        AND nhom6 >= 14 AND phan_bai = 8 AND co_cau = 2 AND migration = 12
