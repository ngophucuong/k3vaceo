...
for (const l of dong.slice(1)) {
  ...
  // 0028 = 138 + Nguyễn Tuấn Hùng migration 0029 = 139 + bảy người migration
  // 0030 = 146). Không phải lưới an toàn duy nhất: KHOP bên dưới còn đối tên,
  // seq sai hẳn (âm, chữ) vẫn bị chặn ở đây trước khi chạm SQL.
  const SEQ_TOI_DA = 146;
